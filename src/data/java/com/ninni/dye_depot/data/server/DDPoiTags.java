...
public class DDPoiTags extends IntrinsicHolderTagsProvider<PoiType> {
    ...
    public DDPoiTags(PackOutput output, CompletableFuture<HolderLookup.Provider> lookup) {
        super(output, Registries.POINT_OF_INTEREST_TYPE, lookup, keyProvider(), DyeDepot.MOD_ID);
    }

    @Override
    protected void addTags(HolderLookup.Provider provider) {
        var poiTypes = provider.lookupOrThrow(Registries.POINT_OF_INTEREST_TYPE);

        tag(DDTags.BEDS)
                .add(DDPoiTypes.HOME.get())
                .add(poiTypes.getOrThrow(PoiTypes.HOME).value());
